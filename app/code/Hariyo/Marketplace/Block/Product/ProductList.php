...
class ProductList extends \Hariyo\Marketplace\Block\Product\Base {
    ...
    public function _prepareLayout() {
        ...
        $this->setChild(
            'attribute_selection', $this->getLayout()->createBlock('\Hariyo\Marketplace\Block\Product\Attributeselection')
        );
    }
    ...
    public function getSellerProduct() {
        ...
        if ($data['count'] > 0) {
            foreach ($product_arr as $product) {
                ...
                $product_name = '<a href="' . $pro->getProductUrl() . '" title="' . __('click to view product details') . '" target="_blank">' . $pro->getName() . '</a> (' . $this->_objectManager->get("\Hariyo\Marketplace\Helper\Product")->getAttributeSetName($product['attribute_set_id']) . ')';
                if (!$this->_objectManager->get("\Magento\Catalog\Helper\Product")->canShow((int) $pro->getId()) || !$pro->isSalable()) {
                    $product_name = $pro->getName() . ' ' . '(' . $this->_objectManager->get("\Hariyo\Marketplace\Helper\Product")->getAttributeSetName($product['attribute_set_id']) . ')';
                }
