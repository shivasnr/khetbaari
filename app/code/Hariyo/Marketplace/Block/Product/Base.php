...
class Base extends \Magento\Framework\View\Element\Template {
    ...
    public function isProductApprovalRequired() {
        try {
            ...
            if ($productId) {
                $prod_to_seller_model = $this->_objectManager->create('\Hariyo\Marketplace\Model\Product')->load($productId, 'product_id');
                $prod_approved = $prod_to_seller_model->getApproved();
                $prod_to_seller_model->unsetData();
                if ($prod_approved == \Hariyo\Marketplace\Helper\GridAction::APPROVED) {
                    return false;
                } else {
                    return true;
                }
    ...
    public function renderFieldHtml($field, $input_name = 'product', $has_event = false) {
        ...
        $field_helper = $this->_objectManager->get('\Hariyo\Marketplace\Helper\Fields');
        $product = $this->getProduct();
        ...
        if ($field['field_input'] == 'textarea') {
            ...
            if ($field['code'] == 'short_description') {
                $maxlength .= 'maxlength="' . \Hariyo\Marketplace\Block\Product\Section\Section::getShortDescriptionLength() . '"';
            } else if ($field['code'] == 'description') {
                $maxlength .= 'maxlength="' . \Hariyo\Marketplace\Block\Product\Section\Section::getLongDescriptionLength() . '"';
            }
    ...
    public function getFrontUrl($controller, $action = '', $params = []){
        ...
        return $this->_objectManager->get("\Hariyo\Marketplace\Helper\Data")->getFrontUrl($controller, $action, $params);
    }
    
    public function getFieldDetail($code) {
        return $this->_objectManager->get("\Hariyo\Marketplace\Helper\Fields")->getFieldDetail($code);
    }
    
    public function getFieldKey($custom_code) {
        return $this->_objectManager->get("\Hariyo\Marketplace\Helper\Fields")->getFieldKey($custom_code);
    }
    
    public function getFormBlocks(){
        return $this->_objectManager->get("\Hariyo\Marketplace\Helper\Fields")->getFormBlocks();
    }
