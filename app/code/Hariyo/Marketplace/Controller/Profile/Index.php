    ...
    public function execute() {
        ...
        try {
            ...
            if ($this->mp_request->isPost() && !empty($post_data)) {
                ...
                $mediaDirectory = $mediaUrl .'Hariyo_Marketplace';
                ...
                if (isset($mpLogo_Img['name']) && ( file_exists($mpLogo_Img['tmp_name'])) && $uploadlogo) {
                    ...
                    $uploader->save(
                        $mediaDirectory
                            ->getAbsolutePath('Hariyo_Marketplace'.'/Seller_' . $this->_customerInfo['entity_id'])
                    );
                    
                    $logoUrl = $mediaUrl. 'Hariyo_Marketplace'. '/Seller_' . $this->_customerInfo['entity_id']. "/" . 'logo.' . $ext;
                ...
                if (isset($mpBanner_Img['name']) && ( file_exists($mpBanner_Img['tmp_name'])) && $uploadBanner) {
                    $path = $mediaUrl .'Hariyo_Marketplace' . '/Seller_' . $this->_customerInfo['entity_id'];
                    $mask = $path . '/banner.*';
                    $matches = glob($mask);
                    if (!empty($matches)){
                        array_map('unlink', $matches);
                    ...
                    $bannerUrl = $mediaUrl. 'Hariyo_Marketplace'. '/Seller_' . $this->_customerInfo['entity_id']. "/" . 'banner.' . $ext;
                    $profile_data['banner'] = $bannerUrl;
                }
                $mpDataHelper = $this->_objectManager->create("\Hariyo\Marketplace\Helper\Data");
    ...
    public function removeLogoAction($seller_id) {
        ...
        try {
            if ($seller_id > 0) {
                ...
                $logoPath = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath('Hariyo_Marketplace'.'/Seller_' . $this->_customerInfo['entity_id']). "/" . $logoFileName;
    ...
    public function removeBannerAction($seller_id) {
        ...
        try {
            if ($seller_id > 0) {
                ...
                $bannerPath = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath('Hariyo_Marketplace'.'/Seller_' . $this->_customerInfo['entity_id']) . "/" . $bannerFileName;
