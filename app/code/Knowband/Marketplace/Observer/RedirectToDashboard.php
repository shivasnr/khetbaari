...
//            $is_seller = $this->mp_sellerHelper->isSeller($customer_data['entity_id']);
            ...
            if ($isCustomerLoggedIn) {
                ...
                $settings_helper = $this->_objectManager->get("Knowband\Marketplace\Helper\Setting");
                $settings = $settings_helper->getSettings();
                if($is_seller && isset($settings['enable_mp']) && ($settings['enable_mp'] == 1)){
                    $CustomRedirectionUrl = $this->_url->getUrl('marketplace/index');
...
//            }
            ...
            $this->mp_logHelper->createFileAndWriteLogData(
                    \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Observer RedirectToDashboard::execute()', $e->getMessage()
            );
        }
    }
}
