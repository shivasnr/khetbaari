...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function saveInitialSellerData($customer_data, $createdFromBackEnd = false) {
        try {
            ...
            if (!empty($check_exist->getData())) {
                ...
                $model->setRegisterLimit(\Knowband\Marketplace\Helper\Setting::REGISTER_REQUEST_LIMIT);
            ...
            $this->mp_logHelper->createFileAndWriteLogData(
                    \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Helper Data::saveInitialSellerData()', $e->getMessage()
            );
            return false;
        }
        return true;
    ...
    public function saveSellerProfileData($profile_data, $seller_id) {
        try {
            ...
            $this->mp_logHelper->createFileAndWriteLogData(
                            \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Helper Data::saveSellerProfileData()', $ex->getMessage()
            );
        }
    }
   ...
   public function saveSellerSettings($setting_data, $sellerId = 0) {
        try {
            ...
            if (empty($selected_category)) {
                $this->mp_productHelper->updateCatgoryMapping([], \Knowband\Marketplace\Helper\Product::CAT_MAP, $category_setting_is, $sellerId);
            } else if (!empty($selected_category)) {
                $disable_cats = $this->mp_settingHelper->getDisableCategories($selected_category);
                $this->mp_productHelper->updateCatgoryMapping($disable_cats, \Knowband\Marketplace\Helper\Product::CAT_UNMAP, $category_setting_is, $sellerId);
                $this->mp_productHelper->updateCatgoryMapping($selected_category, \Knowband\Marketplace\Helper\Product::CAT_MAP, $category_setting_is, $sellerId);
            }
        } catch (\Exception $ex) {
            $this->mp_logHelper->createFileAndWriteLogData(
                            \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Helper Data::saveSellerSettings()', $ex->getMessage()
                    );
        }
    }
