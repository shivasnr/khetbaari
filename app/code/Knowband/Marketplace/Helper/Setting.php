...
class Setting extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getSettings($key = 'knowband/marketplace/general_settings', $front = false, $scope = "default", $scope_id = 0) {
        ...
        if (($settings_array === false || $settings_array === NULL) && $key == 'knowband/marketplace/general_settings') {
            ...
            if ($front) {
                $this->mp_resource->saveConfig("knowband/marketplace/general_settings", json_encode($default_settings), \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
                $this->mp_resource->saveConfig('vss/marketplace/active', $enable_mp, \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
            } else {
                $this->mp_resource->saveConfig("knowband/marketplace/general_settings", json_encode($default_settings), $scope, $scope_id);
     ...
     public function saveDefaultSellerSettings($customer_data, $store_id) {
        try {
            ...
            $this->mp_logHelper->createFileAndWriteLogData(
                    \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Helper Setting::saveDefaultSellerSettings()', $e->getMessage()
            );
        }
    }
    ...
    */
    public function getSystemCategories()
    {
        ...
        try {
            ...
            $this->logger->critical($e->getMessage());
            $this->mp_logHelper->createFileAndWriteLogData(
                    \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Helper Setting::getSystemCategories()', $e->getMessage()
            );
        }
        return $categories;
    }
