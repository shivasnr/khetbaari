...
class Product extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function __construct(
        ...
        \Knowband\Marketplace\Model\Statusaction $mpStatusActionModel,
        \Knowband\Marketplace\Model\Categorymapping $mpCategorymappingModel,
        \Knowband\Marketplace\Model\Orderitem $mpOrderItemModel,
        \Knowband\Marketplace\Model\Earnings $mpEarningModel,
        \Knowband\Marketplace\Helper\Uploader $mpUploaderHelper
    )
    {
...
//        $this->mp_storeManager->setCurrentStore(\Magento\Store\Model\Store::ADMIN_CODE);
        $product = $this->getProduct();
        $associatedProducts = $product->getTypeInstance()->getAssociatedProducts($product);
//        $this->mp_storeManager->setCurrentStore($this->mp_storeManager->getStore()->getId());
        $product_list = [];
    ...
    public function getRelatedProductList($post_data, $col_order = [], $list_params = []) {
        ...
        $start = 0;
        $limit = \Knowband\Marketplace\Helper\Data::PAGELIMIT;
...
//        Mage::getSingleton('catalog/product_status')->addSaleableFilterToCollection($collection);
    ...
    public function processDataBeforeSave($data) {
        ...
        $sellerHelper = $this->mp_objectManager->get("\Knowband\Marketplace\Helper\Seller");
...
//                    $temp['sample'][$index]['old_file'] = $this->_jsonHelper->jsonDecode($arr['old_file']);
    ...
    public function statusUpdateOnProductApproval($product_id, $product_approval_type, $seller_id) {
        $action_type = \Knowband\Marketplace\Helper\GridAction::ACTION_PRODUCT_APPROVAL;
        ...
        if ($product_approval_type != \Knowband\Marketplace\Helper\GridAction::APPROVED) {
        ...
        } else if ($product_approval_type == \Knowband\Marketplace\Helper\GridAction::APPROVED) {
            if ($action->getSize() == 1) {
    ...
    public function getSellerEnabledProducts($seller_id, $website_id = -1) {
        ...
        try {
            ...
            $productCollection = $this->mp_productSellerModel->getCollection()
                    ...
                    ->addFieldToFilter('approved', (int) \Knowband\Marketplace\Helper\GridAction::APPROVED);
            ...
            $this->mp_logHelper->createFileAndWriteLogData(
                    \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Helper Product::getSellerEnabledProducts()', $e->getMessage()
            );
        }
    ...
    public function statusUpdateOnSellerEnable($seller_id, $seller_enable_type) {
        try {
            $action_type = \Knowband\Marketplace\Helper\GridAction::ACTION_SELLER_ENABLE;
            ...
            if (!empty($chunk_data)) {
                foreach ($chunk_data as $chunk) {
                    if ($seller_enable_type != \Knowband\Marketplace\Helper\GridAction::ENABLED) {
                        $tmp_array = [];
                    ...
                    } else if ($seller_enable_type == \Knowband\Marketplace\Helper\GridAction::ENABLED) {
                        $tmp_array = [];
                        foreach ($chunk as $productData) {
            ...
            $this->mp_logHelper->createFileAndWriteLogData(
                    \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Helper Product::statusUpdateOnSellerEnable()', $e->getMessage()
            );
        }
    }
    ...
    public function updateCatgoryMapping($categories, $mode, $level, $seller_id = 0) {
        try {
            ...
            $this->mp_logHelper->createFileAndWriteLogData(
                    \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Helper Product::updateCatgoryMapping()', $e->getMessage()
            );
        }
    }
    ...
    public function statusUpdateOnSellerApproval($seller_id, $seller_approval_type) {
        try {
            $action_type = \Knowband\Marketplace\Helper\GridAction::ACTION_SELLER_APPROVAL;
            $productColl = $this->mp_productSellerModel->getCollection()
                    ->addFieldToFilter('seller_id', ['eq' => $seller_id]);
            $seller_products = $productColl->getData();
            $chunk_data = $this->getArrayInChunks($seller_products);
            ...
            if (!empty($chunk_data)) {
                foreach ($chunk_data as $chunk) {
                    if ($seller_approval_type != \Knowband\Marketplace\Helper\GridAction::APPROVED) {
                        $tmp_array = [];
                        foreach ($chunk as $productData) {
                            $tmp_array[] = $product_id = $productData['product_id'];
                            $action = $this->mp_statusActionModel->getCollection();
                        ...
                        $this->updateProductStatus($tmp_array, \Magento\Catalog\Model\Product\Attribute\Source\Status::STATUS_DISABLED);
                    } else if ($seller_approval_type == \Knowband\Marketplace\Helper\GridAction::APPROVED) {
                        $tmp_array = [];
                        foreach ($chunk as $productData) {
                            $product_id = $productData['product_id'];
                            $action = $this->mp_statusActionModel->getCollection();
            ...
            $this->mp_logHelper->createFileAndWriteLogData(
                    \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Helper Product::statusUpdateOnSellerApproval()', $e->getMessage()
            );
        }
    }
    ...
    public function saveSellerOrderItem($sellerId, $orderId, $orderItems, $isNew = true) {
        try {
            ...
            if (!$isNew) {
                ...
                $commision = (float) $this->mp_objectManager->get('\Knowband\Marketplace\Helper\Setting')->getSettingByKey($sellerId, 'commission');
            }
            ...
            $this->mp_logHelper->createFileAndWriteLogData(
                    \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Helper Product::saveSellerOrderItem()', $ex->getMessage()
            );
        }
    }
