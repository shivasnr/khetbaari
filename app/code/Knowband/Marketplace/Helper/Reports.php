...
class Reports extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getDashBoardGraphData($start_date, $end_date)
    {
        try {
            ...
            $this->mp_logHelper->createFileAndWriteLogData(
                    \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Helper Reports::getDashBoardGraphData()', $ex->getMessage()
            );
        }
        return $stats;
    ...
    public function getSellerProductFromOrder($order_id)
    {
        try {
            ...
            $this->mp_logHelper->createFileAndWriteLogData(
                    \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Helper Reports::getSellerProductFromOrder()', $ex->getMessage()
            );
        }
        return false;
    }
            ...
            foreach ($seller_ordered_products as $item) {
                try {
                    ...
                    $message = 'Failed to calculate earnings after order placing for order id (#' . $order_id . ') with commision(' . $commision_percent . '%). Error - ' . $e->getMessage();
                        $this->mp_logHelper->createFileAndWriteLogData(
                            \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Helper Reports::saveEarningAndCommission()', $message
                        );
                }
            }
    ...
    public function getFormattedBalanceVariationData($firstStartDate, $firstEndDate, $secondStartDate, $secondEndDate)
    {
        ...
        try {
            ...
            $this->mp_logHelper->createFileAndWriteLogData(
                    \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Helper Reports::getFormattedBalanceVariationData()', $ex->getMessage()
            );
        }
        return $reportData;
    }
    ...
    public function updateEarningOnCancelOrder($order_id)
    {
        try {
            ...
            $this->mp_logHelper->createFileAndWriteLogData(
                            \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Controller DisapproveSellerRequest::execute()', $message
                    );
        }
    }
    ...
    {
        try {
            $data = ['consider_for_calculations' => \Knowband\Marketplace\Helper\Product::CALCULATION_NOT_ALLOWED];
            ...
            if($model->getSize() > 0){
                ...
                foreach ($model_data as $orderModel) {
                    ...
                    $this->_eventManager->dispatch('update_order_item_on_cancel', ['order_id' => $order_id, 'consider_for_calculation' => \Knowband\Marketplace\Helper\Product::CALCULATION_NOT_ALLOWED]);
            ...
            $this->mp_logHelper->createFileAndWriteLogData(
                            \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Helper Reports::updateOrderItemsOnCancelOrder()', $message
                    );
        }
    }
