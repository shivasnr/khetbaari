...
class Edit extends ParentController {
    ...
    public function execute() {
        ...
        $this->validateParamId("\Knowband\Marketplace\Model\Product", ['product_id' => $this->getRequest()->getParam('id')]);
        ...
        if ($process_step == 4) {
            ...
            if ($mode && $this->_edit_mode) {
                ...
                $resultRedirect->setPath('marketplace/product/edit', ['id' => $this->getRequest()->getParam('id')]);
        ...
        $resultPage->getConfig()->getTitle()->prepend(__("Seller's Products") . ' - ' . ($product->getId()) ? $product->getName() : __('New Product'));
    ...
    private function save() {
        ...
        $data = $this->_objectManager->get("\Knowband\Marketplace\Helper\Product")->processDataBeforeSave($postedData);
        if ($data) {
            ...
            if ($product->getTypeId() == \Magento\Catalog\Model\Product\Type::TYPE_BUNDLE) {
                ...
                if (isset($bundle_data['bundle_options']['bundle_options'])) {
                    ...
                    $this->_objectManager->get("\Knowband\Marketplace\Helper\Product")->processBundleOptionsData($product);
                    $this->_objectManager->get("\Knowband\Marketplace\Helper\Product")->processDynamicOptionsData($product);
                }
...
//                    }
            ...
            try {
                ...
                $product->save();
                $this->_eventManager->dispatch(
                        'seller_product_save_after', ['product' => $product, 'add_mapping' => &$add_mapping, 'product_id' => $product->getId(), 'seller_id' => $this->_customerInfo['entity_id']]
                );
...
//                }
                ...
                $this->mp_logHelper->createFileAndWriteLogData(
                    \Knowband\Marketplace\Helper\Log::INFOTYPEERROR, 'Controller Product\Edit::save()', $message
                );
