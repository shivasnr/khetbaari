...
namespace Knowband\Marketplace\Controller\Earnings;
...
class GetAjaxData extends ParentController {
    ...
    public function execute() {
        ...
        if ($action == 'history') {
            $data = $this->_layout->getBlockSingleton(\Knowband\Marketplace\Block\Earnings\History::class)->getList();
        } else if ($action == 'orderwise') {
            $data = $this->_layout->getBlockSingleton(\Knowband\Marketplace\Block\Earnings\Orderwise::class)->getList();
        } else if ($action == 'transactionhistory') {
            $data = $this->_layout->getBlockSingleton(\Knowband\Marketplace\Block\Earnings\Transactions::class)->getList();
        }
         $json = [
