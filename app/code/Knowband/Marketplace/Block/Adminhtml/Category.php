...
class Category extends \Magento\Catalog\Block\Adminhtml\Category\AbstractCategory
{
    ...
    public function __construct(
        ...
        \Knowband\Marketplace\Model\Settings $settingsModel,
        array $data = []
    ) {
        ...
        $this->mp_settingsModel = $settingsModel;
    ...
    public function getCategoryIds()
    {
        ...
        if (!$this->getRequest()->getParam('id')) {
            $plugin_data = $this->mp_settingHelper->getSettings();
            
            if (isset($plugin_data['category_ids'])){
                $ids = explode(',', $plugin_data['category_ids']);
            }

            $this->_categoryIds = $ids;
        }
        else {
            $ids = $this->getSellersCategories();
            $this->_categoryIds = $ids;
        }
        return $this->_categoryIds;
    }
    
    public function getSellersCategories() {
        $cat_ids = '';
        $settingsColl = $this->mp_settingsModel->getCollection();
        $settingsColl->addFieldToFilter('seller_id', ['eq' => $this->getRequest()->getParam('id')])
                ->addFieldToFilter('field_name', ['eq' => 'category_ids']);
        $row = $settingsColl->getFirstItem()->getData();
        $global_settings = $this->mp_settingHelper->getSettings();

        if (!empty($row) && isset($row['field_value'])) {
            if (isset($row['use_default']) && $row['use_default']) {

                $cat_ids = isset($global_settings['category_ids']) ? $global_settings['category_ids'] : '';
            } else {
                $cat_ids = $row['field_value'];
            }
        } else {
            $cat_ids = isset($global_settings['category_ids']) ? $global_settings['category_ids'] : '';
        }
        $cats = array_unique(explode(',', $cat_ids));
